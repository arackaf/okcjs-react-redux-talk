...
class Main extends React.Component {
    ...
    componentDidMount(){
        ...
        setTimeout(() => this.props.dispatch({ type: 'SUBJECTS_LOADED', subjects: subjects }), 500);
    }
    toggleBook(_id){
                ...
                <table>
                    ...
                    <tbody>
                    {
                        this.props.books.map(b =>
                            <tr>
                                <td><input type="checkbox" checked={!!this.props.selectedBooks[b._id]} onClick={() => this.toggleBook(b._id)} /> </td>
                                <td>{b.title}</td>
                                <td>{b.author}</td>
                                <td><ul>{b.subjects.map(s => <li>{s.name}</li>)}</ul></td>
                                <td><button onClick={() => this.props.dispatch({ type: 'REMOVE_BOOK', _id: b._id })}>Remove</button></td>
                            </tr>
                        )
                    }
                    </tbody>
